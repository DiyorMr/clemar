    ...
    <>
      ...
      <div className="grid grid-cols-12 gap-5">
        {loading
          ...
          : data.map((item, index) => (
              <NavLink
                className=" col-span-4 my-4 rounded-3xl border-2 p-4 "
                key={index}
                to={`${location.pathname}/${item.id}`}
              >
                <h1 className="text-[#000000] font-monserrat text-xl font-medium leading-6 w-[202px]">
                  {item.title}
                </h1>
                <img src={item.img} alt="" className="pt-4" />
                <p className="text-[#3A3A3A] font-monserrat text-xs font-normal leading-4">
                  {item.text}
                </p>
              </NavLink>
            ))}
